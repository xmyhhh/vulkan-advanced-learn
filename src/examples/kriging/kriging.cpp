...
public:
	...
	concurrency::concurrent_vector <DataPoint> dataPoint_value;
	SceneObject glowsphere;
	SceneObject sphere;

	void prepare()
	{
		KrigeZoneAData(dataPoint, dataPoint_value);
		std::cout << "KrigeZoneAData end";
	...
	void loadAssets()
	{
		...
		std::vector<std::string> obj_to_load = { };
		std::vector<glm::vec3> obj_pos = {  };
		std::vector<glm::vec3> obj_size = {};

		for (auto& const point : dataPoint) {
			obj_to_load.push_back("glowsphere");
			obj_pos.push_back(glm::vec3(point.x / 100, -point.value, point.y / 100));
			obj_size.push_back(glm::vec3(0.2, 0.2, 0.2));
		}


		for (auto& const point : dataPoint_value) {
			obj_to_load.push_back("sphere");
			obj_pos.push_back(glm::vec3(point.x , -point.value, point.y ));
			obj_size.push_back(glm::vec3(0.2, 0.2, 0.2));
		}
		...
		glowsphere.model.loadFromFile(getAssetPath() + "models/glowsphere.gltf", vulkanDevice, queue, glTFLoadingFlags);
		sphere.model.loadFromFile(getAssetPath() + "models/sphere.gltf", vulkanDevice, queue, glTFLoadingFlags);

		//Step 3:define scene 
		scene.objects.resize(obj_to_load.size());
		for (int i = 0; i < obj_to_load.size(); i++) {
			if (obj_to_load[i] == "glowsphere")
				scene.objects[i].model = glowsphere.model;
			else
				scene.objects[i].model = sphere.model;
